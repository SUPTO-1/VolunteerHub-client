...
const ManageVolunteer = () => {
    ...
    return (
        <div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 mb-10 px-2 md:px-6">
            {
                posts.map((post)=><MyPost key={post._id} posts={posts} setPosts={setPosts} post={post}></MyPost>)
            }
           </div>

           <h2 className="text-3xl font-semibold text-poppins text-center mt-10 mb-10">My Volunteer Requests: {volunteerRequest.length}</h2>
        </div>
    );
};
